...
interface SupplyDetailViewProps {
    supply: SupplyItem;
    onDelete: () => void;
    onUpdate: () => void;
    onCancel: () => void;
...
export function SupplyDetailView({ supply, onDelete, onUpdate, onCancel }: SupplyDetailViewProps) {
    return (<div className="space-y-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_1px_0_rgba(15,23,42,0.03)] sm:p-6">
        <div className="flex items-start justify-between gap-4">
            <div>
                <h3 className="text-lg font-semibold text-slate-900">Supply detail</h3>
                <p className="text-sm text-slate-500">Review supply identity, assignment, and current state.</p>
            </div>
            <Badge className={statusClassMap[supply.status]} variant="outline">
                {statusLabelMap[supply.status]}
            </Badge>
        </div>

        <div className="grid gap-5 rounded-xl border border-slate-100 bg-slate-50/40 p-4 sm:p-6 lg:grid-cols-2">
            <div className="space-y-2">
                <Label htmlFor="detail-supply-name" className="text-sm font-semibold text-slate-800">
                    Supply name
                </Label>
                <Input
                    id="detail-supply-name"
            ...
            <div className="space-y-2">
                ...
                <Input
                    ...
                    disabled
                    readOnly
                    value={formatCurrency(supply.unitCost)}
                />
            </div>
            <div className="space-y-2 lg:col-span-2">
                ...
                <Input
                    ...
                    disabled
                    readOnly
                    value={formatCurrency(supply.privatePayRate)}
                />
            </div>
            <div className="space-y-2 lg:col-span-2">
                ...
                <Textarea
                    ...
                    disabled
                    readOnly
                    value={`${statusLabelMap[supply.status]} supply is currently tracked in the inventory register.`}
                />
            </div>
        </div>
